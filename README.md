...
**`updatedots`** — `git pull` on the repo, `genssh`, re-renders `~/.claude/settings.json` from the template and re-registers the `neal-todos`/`jobbot`/`stonks-mcp` MCP servers via `claude mcp add` (token step skips with a message if 1Password CLI isn't installed/signed in — see `claude/claude_settings.sh`), then re-sources `~/.zshrc`.
...
- **MCP server tokens** (`neal-todos`, `jobbot`, `stonks-mcp`) are never stored in this repo. `claude/claude_settings.sh` reads each token from a 1Password *reference* (`op://Private/to-do-mcp/token`, `op://Private/jobbot-mcp/token`, `op://Private/stonks-mcp/token`) at bootstrap/`updatedots` time and registers the server via `claude mcp add` into `~/.claude.json`, which is never written back into the repo. If `op` isn't installed or you're not signed in, it falls back to the previously-registered token, or a placeholder if there is none.
...
| `claude/claude_settings.sh` | Writes `~/.claude/settings.json` from `claude_settings.json.tpl`, registers `neal-todos`/`jobbot`/`stonks-mcp` as user-scope MCP servers via `claude mcp add`, injecting each token from 1Password (preserving the old token if `op` isn't signed in), then symlinks `CLAUDE.md`/`agents`. Sourced by `mac_bootstrap.sh`/`linux_bootstrap.sh`/`updatedots` — expects `$DOTFILES` and `ok`/`info` helpers already defined by the caller. | — |
| `claude/claude_settings.json.tpl` | Template for Claude Code settings (permission allow/deny lists, hooks). Note: MCP servers are *not* configured here — Claude Code reads those from `~/.claude.json`, written by `claude mcp add` in `claude_settings.sh`. Copied to `~/.claude/settings.json` in `updatedots`/bootstrap. | copies to `~/.claude/settings.json` (not committed) |
